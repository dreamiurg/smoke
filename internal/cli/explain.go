...
func runExplain(_ *cobra.Command, _ []string) error {
	...
	fmt.Println()
	fmt.Println("  smoke post <message>     Post a thought (max 280 chars)")
	...
	fmt.Println()
	fmt.Println("Override identity with: smoke post --author \"custom-name\" \"message\"")
	fmt.Println("Or set SMOKE_AUTHOR environment variable.")
	fmt.Println()
